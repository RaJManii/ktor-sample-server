...
class Room(
    ...
    init {
        setPhaseChangeListener { newPhase ->
            when (newPhase) {
                ...
                Phase.SHOW_WORLD -> showWorld()
            }
        }
    }

    private suspend fun sendCurrentDrawInfoToPlayer(player: Player) {
        if (phase == Phase.GAME_RUNNING || phase == Phase.SHOW_WORLD) {
    ...
    suspend fun addPlayer(clientId: String, userName: String, socket: WebSocketSession): Player {
        ...
        if (players.size == 1) phase = Phase.WAITING_FOR_PLAYERS
        ...
        val announcement = Announcement(
            "$userName joined the party!", System.currentTimeMillis(), Announcement.TYPE_PLAYER_JOINED
        )
        sendWordToPlayer(player)
        broadcastPlayerStates()
        sendCurrentDrawInfoToPlayer(player)
    ...
    private fun timeAndNotify(ms: Long) {
        ...
        timerJob = GlobalScope.launch {
            ...
            phase = when (phase) {
                ...
                Phase.SHOW_WORLD -> Phase.NEW_ROUND
                Phase.NEW_ROUND -> {
                    word = null
                    Phase.GAME_RUNNING
                }
    ...
    private fun waitingForPlayers() {
        GlobalScope.launch {
            val phaseChange = PhaseChange(
                Phase.WAITING_FOR_PLAYERS, DELAY_WAITING_FOR_START_TO_NEW_ROUND
            )
            broadcast(gson.toJson(phaseChange))
        }
    }
    ...
    private fun gameRunning() {
        ...
        GlobalScope.launch {
            broadcastToAllExcept(
                gson.toJson(gameStateForGuessingPlayers), drawingPlayer?.clientId ?: players.random().clientId
            )
            drawingPlayer?.socket?.send(Frame.Text(gson.toJson(gameStateForDrawingPlayer)))
            timeAndNotify(DELAY_GAME_RUNNING_TO_SHOW_WORD)

            println("Drawing phase in room $name started. I'll last ${DELAY_GAME_RUNNING_TO_SHOW_WORD / 1000}s")
        }
    }
    ...
    private fun showWorld() {
        GlobalScope.launch {
            if (winningPlayers.isEmpty()) {
                drawingPlayer?.let {
                    it.score -= PENALITY_NOBODY_GUSSED_IT
                }
            }
            broadcastPlayerStates()
            word?.let {
            ...
            val phaseChange = PhaseChange(Phase.SHOW_WORLD, DELAY_SHOW_WORD_TO_NEW_ROUND)
            broadcast(gson.toJson(phaseChange))
        }
    }
        ...
        if (isGuessCorrect(message)) {
            ...
            if (isRoundOver) {
                val roundOverAnnouncement = Announcement(
                    "Everybody gussed it! Now round is starting...",
                    System.currentTimeMillis(),
    ...
    private suspend fun sendWordToPlayer(player: Player) {
        val delay = when (phase) {
            ...
            Phase.SHOW_WORLD -> DELAY_SHOW_WORD_TO_NEW_ROUND
            else -> 0L
        }
        ...
        word?.let { curWord ->
            drawingPlayer?.let { drawingPlayer ->
                val gameState = GameState(
                    ...
                    if (player.isDrawing || phase == Phase.SHOW_WORLD) {
                        curWord
                    } else {
    ...
    enum class Phase {
        WAITING_FOR_PLAYERS, WAITING_FOR_START, NEW_ROUND, GAME_RUNNING, SHOW_WORLD
    }
    ...
    companion object {
        ...
        const val PENALITY_NOBODY_GUSSED_IT = 50
